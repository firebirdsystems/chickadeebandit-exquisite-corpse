...
| One panel per artist per drawing | `segments` → `unique_per_member` (scope `round_id`) |
...
the backend**. This is safe: panels are sealed and `unique_per_member` allows exactly
